/*
  ...
  1. New Tables
    ...
    - `rate_limit_buckets` - Manages rate limiting for API keys

  2. Enhanced Fields for api_keys
    - Key versioning and rotation tracking
    - Advanced security features (IP whitelisting, user agent patterns)
...
CREATE INDEX IF NOT EXISTS idx_api_keys_revoked ON api_keys(is_revoked) WHERE is_revoked = false;
